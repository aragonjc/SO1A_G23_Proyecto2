...
func CasoNuevo(w http.ResponseWriter, r *http.Request) {
	...
	var jsonstr = string(`{"name":"` + nuevo.Name + `","location":"` + nuevo.Location + `","gender":"` + nuevo.Gender + `","age":` + strconv.Itoa(nuevo.Age) + `,"vaccine_type":"` + nuevo.VaccineType + `","Type":"gRPC"` + `}`)
...
func main() {
	...
	router.HandleFunc("/redispub", CasoNuevo).Methods("POST")
